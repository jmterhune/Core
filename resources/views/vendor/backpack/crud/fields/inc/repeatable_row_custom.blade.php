...
@endif
<style>
    .container-repeatable-elements .controls a {
        height: 30px;
        width: 30px;
        border-radius: 50%;
...
<div class="col-md-12 well repeatable-element row m-1 p-2" data-repeatable-identifier="{{ $field['name'] }}">
    ...
    <div class="controls">
        @if($field['name'] != 'template_order_manual')
         <a class="close" style="cursor:pointer" href="{{ backpack_url() . '/event/' . $row['id'] . '/edit' }}"><i class="la la-sm la-edit" style="margin-left: 5px;margin-top: 3px;"></i></a>
        @endif
        @if ($field['reorder'])
