            ...
            <div class="card timeline-item-wrap">
                ...
                @if($history->key == 'created_at' && !$history->old_value)
                    <div class="card-header">
                        <strong class="time"><i
                                class="la la-clock"></i> {{ date('h:ia', strtotime($history->created_at)) }}</strong> -
                ...
                @else
                    <div class="card-header">
                        <strong class="time"><i
                                class="la la-clock"></i> {{ date('h:ia', strtotime($history->created_at)) }}</strong> -
                        {{ $history->userResponsible()?$history->userResponsible()->name:trans('revise-operation::revise.guest_user') }} {{ trans('revise-operation::revise.changed_the') }}
                        @if($history->revisionable_type == "App\Models\CalendarPermission" && $history->fieldName() == 'user')
                            ja
                        @else
                            {{ $history->fieldName() }}
                        @endif
                        <div class="card-header-actions">
                            <form class="card-header-action" method="post"
                                  action="{{ url(\Request::url().'revision_timeline.blade.php/'.$history->id.'/restore') }}">
                                {!! csrf_field() !!}
                                <button type="submit" class="btn btn-outline-danger btn-sm restore-btn"
                        ...
                        @switch($history->key)
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert alert-danger"
                                             style="overflow: hidden;">{{ \App\Models\User::find($history->oldValue())->name ?? null }}
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="alert alert-success"
                                             style="overflow: hidden;">{{ \App\Models\User::find($history->newValue())->name ?? null}}
                                        </div>
                                    </div>
                                </div>
                            @break
                            @case('judge_id')
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert alert-danger"
                                             style="overflow: hidden;">{{ \App\Models\Judge::find($history->oldValue())->name ?? null }}</div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="alert alert-success"
                                             style="overflow: hidden;">{{ \App\Models\Judge::find($history->newValue())->name ?? null}}</div>
                                    </div>
                                </div>
                                @break
                            @case('def_attorney_id')
                            @case('opp_attorney_id')
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert alert-danger"
                                             style="overflow: hidden;">{{ \App\Models\Attorney::find($history->oldValue())->name ?? null }}</div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="alert alert-success"
                                             style="overflow: hidden;">{{ \App\Models\Attorney::find($history->newValue())->name ?? null}}</div>
                                    </div>
                                </div>
                                @break
                            @case('county_id')
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert alert-danger"
                                             style="overflow: hidden;">{{ \App\Models\County::find($history->oldValue())->name ?? null }}</div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="alert alert-success"
                                             style="overflow: hidden;">{{ \App\Models\County::find($history->newValue())->name ?? null}}</div>
                                    </div>
                                </div>
                            ...
                            @default
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="alert alert-danger"
                                             style="overflow: hidden;">{{ $history->oldValue() }}</div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="alert alert-success"
                                             style="overflow: hidden;">{{ $history->newValue() }}</div>
                                    </div>
                                </div>
                            @break
                        @endswitch
